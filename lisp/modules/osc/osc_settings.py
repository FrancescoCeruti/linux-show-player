...
class OscSettings(SettingsPage):
    ...
    def __init__(self, **kwargs):
        ...
        self.enableModule.setText(translate('OscSettings', 'enable OSC'))
        ...
        self.enableModule.stateChanged.connect(
            self.activate_module,Qt.QueuedConnection)
        self.inportBox.valueChanged.connect(
            self.change_inport, Qt.QueuedConnection)
        self.outportBox.valueChanged.connect(
            self.change_outport, Qt.QueuedConnection)
        self.hostnameEdit.textChanged.connect(
            self.change_hostname, Qt.QueuedConnection)
    ...
    def change_inport(self):
        port = str(self.inportBox.value())
        if port != config.get('OSC', 'inport'):
            config.set('OSC', 'inport', port)
            if self.enableModule.isChecked():
                self.enableModule.setChecked(False)

    def change_outport(self):
        config.set('OSC', 'outport', str(self.outportBox.value()))
    ...
    def load_settings(self, settings):
        ...
        self.inportBox.setValue(int(settings.get('inport', 9000)))
        self.outportBox.setValue(int(settings.get('outport', 9001)))
        self.hostnameEdit.setText(settings.get('hostname', 'localhost'))
