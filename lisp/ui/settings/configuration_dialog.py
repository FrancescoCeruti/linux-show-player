...
class ConfigurationDialog(QDialog):
    '''
    Note for implementing subclasses of this:
    1) Define a named tuple named "PageEntry" (not a class member)
    2) Define a class member named "PagesRegistry" of type "DictNode"
    3) Define a static method named "_getConfigFromPageEntry"
       which returns the config from step (1)
    '''
    ...
    def __init__(self, **kwargs):
        ...
        for page_node in self.__class__.PagesRegistry.children:
    ...
    def _populateModel(self, model_parent, page_parent):
        if page_parent.value is not None:
            ...
            config = self._getConfigFromPageEntry(page_parent)
        else:
            page_class = None
            config = None
    ...
    def _getConfigFromPageEntry(page_parent):
        '''This needs overridding by any subclass.'''
        logger.critical(
            translate(
                "ConfigurationCritical",
                "Need to implement the Static Method `_getConfigFromPageEntry`"
            )
        )
        return None

    @classmethod
    def unregisterSettingsPage(cls, path):
        """
        :param path: indicate the page "position": 'category.sub.key'
        :type path: str
        """
        cls.PagesRegistry.pop(path)
